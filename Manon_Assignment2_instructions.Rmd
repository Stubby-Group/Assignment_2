...
```{r}
library(tidyverse, brms, rethinking)
#install.packages(rstan)
d <- read.csv("Ass2.csv")
print(paste(mean(d$VoiceHearing),
            mean(d$MindReading),
            mean(d$Apathy),
...
model_1 <- brm(data = d,
               ...
               prior = c(prior(normal(0, 0.2), class = Intercept),
                         ...
                         prior(exponential(1), class = sigma))) #prior(normal(sd(scaledOutcomevar, 0.5)), class = sigma)

summary(model_1)

get_prior(AltercentricIntrusion ~ 1 + Diagnosis, data = d, family = gaussian) # this is the default priors for the model?
...
d_patients$VoiceHearing <- standardize(d_patients$VoiceHearing)
d_patients$MindReading <- standardize(d_patients$MindReading)
d_patients$Apathy <- standardize(d_patients$Apathy)
d_patients$AltercentricIntrusion <- standardize(d_patients$AltercentricIntrusion)
...
diagnosis_dag <- dagitty( "dag {
    Diagosis -> VoiceHearing
    Diagosis -> AltercentricIntrusion
    Diagosis -> MindReading
    Diagosis -> Apathy
}")
...
coordinates(diagnosis_dag) <- list(x=c(VoiceHearing = 1, Apathy = 2, Diagosis = 2, AltercentricIntrusion = 2, MindReading = 3),
                                   y=c(Diagosis = 1, VoiceHearing = 2, Apathy = 2, MindReading = 2, AltercentricIntrusion = 13
