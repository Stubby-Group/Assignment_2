...
```{r}
pacman::p_load(tidyverse, rethinking, brms)
...
```{r}
#ploting the data to see the distribution 
...
# making models 
m1 <- bf(AltercentricIntrusion ~  Diagnosis +
              (1 | ID))

# Modeling the data without priors and stuff
m2 <- bf(AltercentricIntrusion ~ 1 + Diagnosis + 
              (1 | ID))  


#Priors
get_prior(m1, dstand, family = gaussian) #We need a prior for the intercept, one for beta, a prior for the sigma, and a prior for the sd:
...
#ll_f <- bf(df$AltercentricIntrusion ~ df$Diagnosis)
...
plot(conditional_effects(m2_priorCheck), spaghetti = T, 
     method = "predict", 
     nsamples = 100, 
     points = T) 
plot(conditional_effects(m2_priorCheck), spaghetti = T, 
